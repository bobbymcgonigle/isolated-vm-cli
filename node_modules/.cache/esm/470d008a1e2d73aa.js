let ivm,assert,fs,path,chalk;_1e2‍.x([["compileAndExecute",()=>compileAndExecute],["printResult",()=>printResult],["processScriptOptions",()=>processScriptOptions]]);_1e2‍.w("isolated-vm",[["default",["ivm"],function(v){ivm=v}]]);_1e2‍.w("assert",[["default",["assert"],function(v){assert=v}]]);_1e2‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_1e2‍.w("path",[["default",["path"],function(v){path=v}]]);_1e2‍.w("chalk",[["default",["chalk"],function(v){chalk=v}]]);
  ...
  // Console hook start.
  if(debugMode) {
    _1e2‍.g.console.stdlog = _1e2‍.g.console.log.bind(_1e2‍.g.console);
    _1e2‍.g.console.logs = [];
    _1e2‍.g.console.log = function(){
      _1e2‍.g.console.logs.push(Array.from(arguments));
    }
  }
  // Console hook end.

  _1e2‍.g.console.log("Filename: %s\nLogs: %s\nResult: %s\n", result.filename, result.logs.toString(), result.result);
  ...
  if(printIsolateStats && result.cpuTime && result.wallTime ) {
    ...
    _1e2‍.g.console.log("Heap Stats: %s\nCpu time: %s\nWall time: %s", result.heapStats, cpuInMil, wallInMil);
  }
  return _1e2‍.g.console.logs;
}
