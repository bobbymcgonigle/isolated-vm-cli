let ivm,assert,fs,path,chalk;_efa‍.x([["compileAndExecute",()=>compileAndExecute],["printResult",()=>printResult],["processScriptOptions",()=>processScriptOptions]]);_efa‍.w("isolated-vm",[["default",["ivm"],function(v){ivm=v}]]);_efa‍.w("assert",[["default",["assert"],function(v){assert=v}]]);_efa‍.w("fs",[["default",["fs"],function(v){fs=v}]]);_efa‍.w("path",[["default",["path"],function(v){path=v}]]);_efa‍.w("chalk",[["default",["chalk"],function(v){chalk=v}]]);
  ...
  try {
    ...
    return { filename, logs, result: err.stack, heapStats, cpuTime, wallTime };
       ...
       function printResult(result, printIsolateStats, debugMode) {
  ...
  // Console hook start.
  if(debugMode) {
    _efa‍.g.console.stdlog = _efa‍.g.console.log.bind(_efa‍.g.console);
    _efa‍.g.console.logs = [];
    _efa‍.g.console.log = function(){
      _efa‍.g.console.logs.push(Array.from(arguments));
    }
  }
  // Console hook end.

  _efa‍.g.console.log("Filename: %s\nLogs: %s\nResult: %s\n", result.filename, result.logs.toString(), result.result);
  ...
  if(printIsolateStats && result.cpuTime && result.wallTime ) {
    ...
    _efa‍.g.console.log("Heap Stats: %s\nCpu time: %s\nWall time: %s", result.heapStats, cpuInMil, wallInMil);
  }
  return _efa‍.g.console.logs;
}
